...
public class GamePanel extends JPanel implements MouseListener {
	...
	public GamePanel(int w, int h, int boom, GameFrame gameFrame) {
		...
		world = new World(w, h, boom, this, gameFrame);
		...
		for (int i = 0; i < arrayButton.length; i++) {
			for (int j = 0; j < arrayButton[i].length; j++) {
				if (e.getButton() == 1 && e.getSource() == arrayButton[i][j] && !world.getArrayCamCo()[i][j]) {
					if (world.arrayBoolean[i][j] == false)
					{
						gameFrame.move++;
					}			
					...
					System.out.println(gameFrame.openCount + " " + gameFrame.move);
					gameFrame.stack2.push(gameFrame.openCount);
					gameFrame.openCount = 0;
